...
- **Two assignment entries — spiritual guidance, never a to-do list.** "Your Assignment" tells someone how to *be*, the way a tarot card speaks; it is not an errand for the evening. Entry one is the orientation (how they are asked to meet the world), entry two the truth underneath it. No times, places, counts or festival logistics — nothing that could be ticked off. Max 48 characters each — one sentence on one printed line, enforced at generation (the route retries with the over-long entry quoted back) and surfaced as a counter in the editor. Stored as `assignments: string[]` in `card.json`.
