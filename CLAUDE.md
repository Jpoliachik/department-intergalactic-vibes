...
- **Current deck face:** full-bleed **square** art band across the top (card width by card width, 71.6% of card height — it matches the 1:1 aspect ratio requested from the image model, so nothing is cropped); name + designation in a bordered plate over the art, top left; Vibe Corp seal circle top right. Below a hairline mustard rule with a diamond ornament straddling it: the tagline pinned static under the rule, then the wisdom passage centred in the remaining space.
- **The wisdom passage** — not a to-do list and not bullet points. exactly two short lines set in an ornate serif, centred, forming ONE passage — the first line opens it, the second turns and lands it — rather than two separate aphorisms. It tells someone how to *be* and why that holds, the way a tarot card speaks. No times, places, counts or festival logistics; nothing that can be ticked off. Max 42 characters per line — each line is authored, not wrapped — enforced at generation (the route retries with the over-long line quoted back) and shown as a counter in the editor. Stored as `assignments: string[]` in `card.json`; the field name predates the concept.
