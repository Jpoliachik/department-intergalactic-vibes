...
- **Two assignments** carry the doing. These cards are handed over by crew in the field, so they read as issued: short imperatives, 2–6 words, max 34 characters, numbered 01/02 in mono. Actionable but never prescriptive — no times, places, counts or named people, and nothing that can be finished and ticked off. A person should be able to act on one ten times in a night. The pair differ in kind: one is the outward move, the other the permission that makes it possible ("Be the first one in" / "Let yourself look silly"). Enforced at generation and shown as a counter in the editor.
