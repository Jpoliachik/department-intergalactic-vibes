...
- **Print margin is mandatory.** Art may bleed to the trim edge. **Readable content never may.** Every text element, plate, code and mark sits inside a safe margin of **7% of the card width** on all four edges — that is the floor, and readable content actually sits at **11%** on the left and right. A trim that drifts should eat artwork, never meaning.
- **Print export.** The studio's *Export for print* button rasterises every card to `card-studio/export/` (gitignored) by screenshotting the same `CardFace` component in headless Chrome — one card face in the codebase, and the printer gets it. Output is **816×1110px, 300dpi**: the 63×88mm trim box plus **3mm of bleed** on every edge. The design is never scaled up to fill the bleed; the trim box stays exactly what the studio shows and the art band and text plate overhang it, so a drifting guillotine eats bleed and every margin survives at full width.
